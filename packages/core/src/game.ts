...
export const BUILDING_MAX_TROOP_BONUS: Record<BuildingType, number> = {
  defensePost: 0,
  port:        0,
...
export const DEFENSE_POST_MULT = 5;
...
function computeMaxTroops(tileCount: number, cityCapBonus: number, isBot: boolean): number {
...
function computeTroopGrowth(troops: number, maxTroops: number, factoryMult: number): number {
...
export function createPlayer(id: string, name: string, colorIndex: number, isBot = false): Player {
...
export function tickGame(state: GameState): GameState {
  ...
  for (const tile of state.tiles) {
    if (tile.owner) tileCounts[tile.owner] = (tileCounts[tile.owner] || 0) + 1;
  }
  ...
  for (const building of Object.values(state.buildings)) {
    ...
    buildingGold[pid]     = (buildingGold[pid]     ?? 0) + BUILDING_GOLD_PER_TICK[building.type];
    buildingCapBonus[pid] = (buildingCapBonus[pid]  ?? 0) + BUILDING_MAX_TROOP_BONUS[building.type];
    if (building.type === 'factory') {
      factMult[pid] = (factMult[pid] ?? 1) * BUILDING_GROWTH_MULT.factory;
    }
  }
  ...
  for (const playerId in state.players) {
    ...
    if (player.isEliminated) { newPlayers[playerId] = player; continue; }
...
export function applyConquer(
  ...
  const player = state.players[playerId];
  if (!player || player.isEliminated) return { error: 'Oyuncu bulunamadı' };

  const tile = state.tiles[payload.tileId];
  if (!tile) return { error: 'Kare bulunamadı' };
  if (!canConquer(tile)) return { error: 'Su kareleri fethedilemez' };
  if (tile.owner === playerId) return { error: 'Bu kare zaten sizin' };
...
export function applyBuild(
  ...
  const tile = state.tiles[payload.tileId];
  if (!tile) return { error: 'Kare bulunamadı' };
  if (tile.owner !== playerId) return { error: 'Bu kare size ait değil' };
  if (!canConquer(tile)) return { error: 'Su karelerine bina kurulamaz' };
  if (state.buildings[payload.tileId]) return { error: 'Burada zaten bir bina var' };
  ...
  if (payload.buildingType === 'port') {
    ...
    if (!isCoastal) return { error: 'Liman yalnızca kıyı karelerine kurulabilir' };
  }
...
export function applyAlliance(state: GameState, playerA: string, playerB: string): GameState {
...
export function breakAlliance(state: GameState, playerA: string, playerB: string): GameState {
  const a = state.players[playerA], b = state.players[playerB];
  if (!a || !b) return state;
  return {
    ...state,
...
export function applyNuke(state: GameState, playerId: string, payload: NukePayload): NukeResult {
  ...
  for (const b of Object.values(state.buildings)) {
    ...
    if (bt) {
      ...
      if (Math.sqrt(dx * dx + dy * dy) <= RADIUS) delete newBuildings[b.tileId];
    }
  }
